...
toList (Cursor first y last) =
    List.foldl (::) (y :: last) first
...
forward (Cursor first y last) =
    case last of 
        [] -> Nothing
        x::xs -> Just (Cursor (y::first) x xs)
...
back (Cursor first y last) =
    case first of
        [] -> Nothing
        x::xs -> Just (Cursor xs x (y::last))
...
-}
length : Cursor a -> Int
length (Cursor first _ last) = 
    List.length first + 1 + List.length last
